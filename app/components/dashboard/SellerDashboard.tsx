...
import { Store, Plus, TrendingUp, DollarSign, PenSquare, Trash2, Book, Mic, FileText, Loader2 } from 'lucide-react';
import ProductForm, { ProductData, ProductType, BookCategory } from './ProductForm';
...
export default function SellerDashboard() {
    ...
    const fetchSellerProducts = async () => {
        ...
        try {
            ...
            // Map DB product to UI ProductData
            const mapped: ProductData[] = fetched.map(p => ({
                id: p.id,
                title: p.title,
                author: p.seller.profile?.name || 'Unknown', // Or derived
                price: p.price.toString(),
                type: p.type.toLowerCase() as ProductType,
                category: p.category ? p.category.replace(/_/g, ' ') as BookCategory : undefined,
    ...
    const handleFormSubmit = async (data: ProductData) => {
        ...
        try {
            // Convert UI data to DB data payload
            // Prisma Enums are typically Uppercase. We need to cast our lowercase UI strings.
            const payload = {
                title: data.title,
                description: data.description,
                price: parseFloat(data.price),
                type: data.type.toUpperCase() as any, // Cast to Prisma Enum type
                category: data.category ? data.category.replace(/ /g, '_') as any : undefined, // Handle spaces if mapped
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            ...
            case 'sermon': return <Mic className="w-6 h-6" />;
    ...
    const getTypeColor = (type: string) => {
        switch (type) {
            ...
            case 'sermon': return 'bg-purple-400';
            case 'article': return 'bg-amber-400';
            default: return 'bg-slate-400';
        }
    };

    if (isLoading && products.length === 0) {
        return <div className="p-12 text-center"><Loader2 className="animate-spin inline-block text-primary" /></div>;
    }
    ...
    return (
        <div className="p-6 relative">
            ...
            {/* Stats Cards - Simplified with real count */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    {/* Mock earnings for now as we don't have orders yet */}
                    <p className="text-2xl font-bold text-secondary">{formatPrice(0)}</p>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-medium text-slate-500">Sales This Month</span>
                    </div>
            ...
            <div className="space-y-4">
                {products.length === 0 ? (
                    <div className="text-center py-12 bg-slate-50 rounded-lg border border-dashed border-slate-300">
                        <p className="text-slate-500">No products found. Start adding some!</p>
                    </div>
                ) : (
                    products.map((product) => (
                        <div key={product.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-100 hover:border-slate-200 transition-colors group">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap text-xs text-slate-500 capitalize">
                                        <span className={`inline-block w-2 h-2 rounded-full ${getTypeColor(product.type)}`}></span>
